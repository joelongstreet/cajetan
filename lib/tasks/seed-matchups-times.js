...
function parsePage({ html, url, matchupId }) {
  console.log(`parsing data from ${url} for matchup ${matchupId}`);
...
async function fetchParseSeed({ matchupId, recapLink }) {
  console.log(`fetching data from ${recapLink}`);

  const html = await fetchHtml(recapLink);

  try {
    const matchupStartTime = parsePage({ html, url: recapLink, matchupId });
    console.log(`inserting start time for matchup ${matchupId}: ${matchupStartTime}...`);
...
execute().then(() => {
  ...
  console.log(`seeded matchup start times in ${timeDiff} seconds`);
  process.exit();
});
