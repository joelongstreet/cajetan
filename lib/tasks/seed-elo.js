...
const leagueCsvUrls = [
  {
    ...
    parseRow      : (row, searchTermMap) => {
      const dateObject  = DateTime.fromFormat(row.date, fromDateTimeFormat);
...
async function readFetchSeed({ league, url, parseRow, getTeamTerms }) {
  const csv             = await fetchCsv(url);
  const rows            = parse(csv, { columns: true });
  const teamTerms       = rows.map(getTeamTerms).flat(2);
  const searchTermMap   = await buildSearchTermMap(league, teamTerms);
  const parsedRows      = rows.map((r) => parseRow(r, searchTermMap));
  const insertReady     = parsedRows.flat(2).filter((r) => (
